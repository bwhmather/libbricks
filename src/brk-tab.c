...
update_needs_attention(BrkTab *self) {
    ...
    brk_timed_animation_set_value_from(
        BRK_TIMED_ANIMATION(self->needs_attention_animation),
        brk_animation_get_value(self->needs_attention_animation)
    );
    brk_timed_animation_set_value_to(
        BRK_TIMED_ANIMATION(self->needs_attention_animation), needs_attention ? 1 : 0
    );
...
update_loading(BrkTab *self) {
    update_icons(self);
    update_spinner(self);
    set_style_class(GTK_WIDGET(self), "loading", brk_tab_page_get_loading(self->page));
}
...
static void
enter_cb(BrkTab *self, double x, double y, GtkEventController *controller) {
    self->hovering = TRUE;
...
static void
motion_cb(BrkTab *self, double x, double y, GtkEventController *controller) {
...
static void
leave_cb(BrkTab *self, GtkEventController *controller) {
    self->hovering = FALSE;
...
static gboolean
drop_cb(BrkTab *self, GValue *value) {
...
brk_tab_measure(
    ...
    if (orientation == GTK_ORIENTATION_HORIZONTAL) {
        ...
        gtk_widget_measure(
            self->icon_stack, orientation, for_size, &child_min, &child_nat, NULL, NULL
        );
        min = MAX(min, child_min);
        nat = MAX(nat, child_nat);

        gtk_widget_measure(self->title, orientation, for_size, &child_min, &child_nat, NULL, NULL);
        min = MAX(min, child_min);
        nat = MAX(nat, child_nat);

        gtk_widget_measure(
            self->close_btn, orientation, for_size, &child_min, &child_nat, NULL, NULL
        );
        min = MAX(min, child_min);
        nat = MAX(nat, child_nat);

        gtk_widget_measure(
            self->indicator_btn, orientation, for_size, &child_min, &child_nat, NULL, NULL
        );
        min = MAX(min, child_min);
        nat = MAX(nat, child_nat);

        gtk_widget_measure(
            self->needs_attention_indicator, orientation, for_size, &child_min, &child_nat, NULL,
            NULL
        );
        min = MAX(min, child_min);
        nat = MAX(nat, child_nat);
...
static inline void
allocate_child(
    GtkWidget *child, int parent_width, int parent_height, int x, int width, int baseline
) {
...
static int
get_attention_indicator_width(BrkTab *self, int center_width) {
    double base_width;
...
static void
brk_tab_size_allocate(GtkWidget *widget, int width, int height, int baseline) {
    BrkTab *self = BRK_TAB(widget);
    ...
    if (gtk_widget_get_visible(self->indicator_btn)) {
        if (self->inverted) {
            allocate_child(
                self->indicator_btn, width, height, width - indicator_width, indicator_width,
                baseline
            );

            end_width = indicator_width;
        } else {
            allocate_child(self->indicator_btn, width, height, 0, indicator_width, baseline);
    ...
    if (gtk_widget_get_visible(self->close_btn)) {
        if (self->inverted) {
            allocate_child(self->close_btn, width, height, 0, close_width, baseline);
    ...
    center_width = MIN(width - start_width - end_width, icon_width + title_width);
    center_x = CLAMP((width - center_width) / 2, start_width, width - center_width - end_width);

    self->close_overlap = !self->inverted && !self->title_inverted &&
        gtk_widget_get_visible(self->title) && gtk_widget_get_visible(self->close_btn) &&
        center_x + center_width > width - close_width;

    needs_attention_width =
        MAX(needs_attention_width, get_attention_indicator_width(self, center_width));
    needs_attention_x = (width - needs_attention_width) / 2;

    allocate_child(
        self->needs_attention_indicator, width, height, needs_attention_x, needs_attention_width,
        baseline
    );

    if (gtk_widget_get_visible(self->icon_stack)) {
        allocate_child(self->icon_stack, width, height, center_x, icon_width, baseline);
...
static void
brk_tab_snapshot(GtkWidget *widget, GtkSnapshot *snapshot) {
    BrkTab *self = BRK_TAB(widget);
    ...
    if (draw_fade) {
        ...
        float offset =
            gtk_widget_get_width(self->close_btn) + gtk_widget_get_margin_end(self->title);
        ...
        gtk_snapshot_append_linear_gradient(
            snapshot, &GRAPHENE_RECT_INIT(0, 0, FADE_WIDTH + offset, height),
            &GRAPHENE_POINT_INIT(offset, 0), &GRAPHENE_POINT_INIT(FADE_WIDTH + offset, 0),
            (GskColorStop[2]){
                {0, {0, 0, 0, opacity}},
                {1, {0, 0, 0, 0}},
            },
...
static void
brk_tab_direction_changed(GtkWidget *widget, GtkTextDirection previous_direction) {
    BrkTab *self = BRK_TAB(widget);

    update_title(self);

    GTK_WIDGET_CLASS(brk_tab_parent_class)->direction_changed(widget, previous_direction);
}
...
brk_tab_constructed(GObject *object) {
    ...
    g_signal_connect_object(
        self->view, "notify::default-icon", G_CALLBACK(update_icons), self, G_CONNECT_SWAPPED
    );
}

static void
brk_tab_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
    BrkTab *self = BRK_TAB(object);
...
static void
brk_tab_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    BrkTab *self = BRK_TAB(object);
...
brk_tab_class_init(BrkTabClass *klass) {
    ...
    props[PROP_DRAGGING] = g_param_spec_boolean(
        "dragging", NULL, NULL, FALSE,
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY
    );

    props[PROP_PAGE] = g_param_spec_object(
        "page", NULL, NULL, BRK_TYPE_TAB_PAGE,
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY
    );

    props[PROP_INVERTED] = g_param_spec_boolean(
    ...
    signals[SIGNAL_EXTRA_DRAG_DROP] = g_signal_new(
        ...
        g_signal_accumulator_first_wins, NULL, NULL, G_TYPE_BOOLEAN, 2, G_TYPE_VALUE,
        GDK_TYPE_DRAG_ACTION
    );

    signals[SIGNAL_EXTRA_DRAG_VALUE] = g_signal_new(
        "extra-drag-value", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
        g_signal_accumulator_first_wins, NULL, NULL, GDK_TYPE_DRAG_ACTION, 1, G_TYPE_VALUE
    );

    g_object_class_install_properties(object_class, LAST_PROP, props);

    gtk_widget_class_set_template_from_resource(
        widget_class, "/com/bwhmather/Bricks/ui/brk-tab.ui"
    );
...
brk_tab_init(BrkTab *self) {
    ...
    brk_timed_animation_set_easing(
        BRK_TIMED_ANIMATION(self->needs_attention_animation), BRK_EASE_IN_OUT_CUBIC
    );
}
...
brk_tab_new(BrkTabView *view) {
    ...
    return g_object_new(BRK_TYPE_TAB, "view", view, NULL);
}
...
void
brk_tab_set_page(BrkTab *self, BrkTabPage *page) {
    ...
    if (self->page) {
        ...
        g_signal_connect_object(
            self->page, "notify::selected", G_CALLBACK(update_selected), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::title", G_CALLBACK(update_title), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::tooltip", G_CALLBACK(update_tooltip), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::icon", G_CALLBACK(update_icons), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::indicator-icon", G_CALLBACK(update_icons), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::indicator-activatable", G_CALLBACK(update_indicator), self,
            G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::needs-attention", G_CALLBACK(update_needs_attention), self,
            G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->page, "notify::loading", G_CALLBACK(update_loading), self, G_CONNECT_SWAPPED
        );
    }
...
void
brk_tab_set_dragging(BrkTab *self, gboolean dragging) {
    g_return_if_fail(BRK_IS_TAB(self));
...
void
brk_tab_set_inverted(BrkTab *self, gboolean inverted) {
    g_return_if_fail(BRK_IS_TAB(self));
...
void
brk_tab_set_fully_visible(BrkTab *self, gboolean fully_visible) {
    g_return_if_fail(BRK_IS_TAB(self));
...
void
brk_tab_setup_extra_drop_target(BrkTab *self, GdkDragAction actions, GType *types, gsize n_types) {
    g_return_if_fail(BRK_IS_TAB(self));
...
void
brk_tab_set_extra_drag_preload(BrkTab *self, gboolean preload) {
    g_return_if_fail(BRK_IS_TAB(self));
