        ...
        for (i = 0; i < focus_order->len; i++) {
            ...
            if (child != old_focus) {
                ...
                if (gtk_widget_compute_bounds(child, widget, &child_bounds)) {
                    ...
                    if ((G_APPROX_VALUE(child_y2, compare_y1, FLT_EPSILON) ||
                         child_y2 < compare_y1 ||
                         G_APPROX_VALUE(child_y1, compare_y2, FLT_EPSILON) ||
                         child_y1 > compare_y2) /* No vertical overlap */
                        || (direction == GTK_DIR_RIGHT &&
                            child_bounds.origin.x + child_bounds.size.width < compare_x
                           ) || /* Not to left */
                        (direction == GTK_DIR_LEFT && child_bounds.origin.x > compare_x
                        )) /* Not to right */ {
        ...
        for (i = 0; i < focus_order->len; i++) {
            ...
            if (child != old_focus) {
                ...
                if (gtk_widget_compute_bounds(child, widget, &child_bounds)) {
                    ...
                    if ((G_APPROX_VALUE(child_x2, compare_x1, FLT_EPSILON) ||
                         child_x2 < compare_x1 ||
                         G_APPROX_VALUE(child_x1, compare_x2, FLT_EPSILON) ||
                         child_x1 > compare_x2) /* No horizontal overlap */
                        || (direction == GTK_DIR_DOWN &&
                            child_bounds.origin.y + child_bounds.size.height < compare_y
                           ) || /* Not below */
                        (direction == GTK_DIR_UP && child_bounds.origin.y > compare_y
                        )) /* Not above */ {
                        g_ptr_array_remove_index(focus_order, i);
                        i--;
                    }
...
static void
focus_sort(GtkWidget *widget, GtkDirectionType direction, GPtrArray *focus_order) {
    GtkWidget *child;
...
static gboolean
focus_move(GtkWidget *widget, GtkDirectionType direction) {
    ...
    for (i = 0; i < focus_order->len && !ret; i++) {
        ...
        } else if (gtk_widget_get_mapped(child) && gtk_widget_is_ancestor(child, widget)) {
            ret = gtk_widget_child_focus(child, direction);
        }
    }
