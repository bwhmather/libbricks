...
G_DEFINE_FINAL_TYPE_WITH_CODE(
    BrkTabBox, brk_tab_box, GTK_TYPE_WIDGET, G_IMPLEMENT_INTERFACE(GTK_TYPE_SCROLLABLE, NULL)
)
...
find_tab_info_at(BrkTabBox *self, double x) {
    GList *l;
...
static inline GList *
find_link_for_page(BrkTabBox *self, BrkTabPage *page) {
    GList *l;
...
static inline TabInfo *
find_info_for_page(BrkTabBox *self, BrkTabPage *page) {
...
static GList *
find_nth_alive_tab(BrkTabBox *self, guint position) {
    GList *l;
...
static int
get_base_tab_width(BrkTabBox *self, gboolean target_end_padding, gboolean target_animations) {
...
static int
predict_tab_width(BrkTabBox *self, TabInfo *info, gboolean assume_placeholder) {
    int n;
...
static void
resize_animation_value_cb(double value, BrkTabBox *self) {
...
set_tab_resize_mode(BrkTabBox *self, TabResizeMode mode) {
    ...
    notify = (self->tab_resize_mode == TAB_RESIZE_NORMAL) != (mode == TAB_RESIZE_NORMAL);
...
static void
motion_cb(BrkTabBox *self, double x, double y, GtkEventController *controller) {
    GdkDevice *device = gtk_event_controller_get_current_event_device(controller);
    GdkInputSource input_source = gdk_device_get_source(device);
...
static void
leave_cb(BrkTabBox *self, GtkEventController *controller) {
    self->hovering = FALSE;
...
static void
focus_tab_cb(BrkTabBox *self, GVariant *args) {
...
static void
reorder_tab_cb(BrkTabBox *self, GVariant *args) {
    GtkDirectionType direction;
    gboolean last, is_rtl, success;
...
static void
add_focus_bindings(
    GtkWidgetClass *widget_class, guint keysym, GtkDirectionType direction, gboolean last
) {
...
drop_switch_timeout_cb(BrkTabBox *self) {
    ...
    brk_tab_view_set_selected_page(self->view, self->drop_target_tab->page);
}
...
animate_scroll(BrkTabBox *self, TabInfo *info, double offset, guint duration) {
    ...
    brk_timed_animation_set_duration(BRK_TIMED_ANIMATION(self->scroll_animation), duration);
    brk_animation_play(self->scroll_animation);
}

static void
animate_scroll_relative(BrkTabBox *self, double delta, guint duration) {
...
static gboolean
scroll_to_tab_full(
    BrkTabBox *self, TabInfo *info, int pos, guint duration, gboolean keep_selected_visible
) {
    int tab_width;
...
static gboolean
scroll_to_tab(BrkTabBox *self, TabInfo *info, guint duration) {
    return scroll_to_tab_full(self, info, -1, duration, FALSE);
}

static gboolean
scroll_cb(BrkTabBox *self, double dx, double dy, GtkEventController *controller) {
...
static void
scroll_animation_cb(double value, BrkTabBox *self) {
    gtk_widget_queue_resize(GTK_WIDGET(self));
}
...
set_hadjustment(BrkTabBox *self, GtkAdjustment *adjustment) {
    ...
    if (self->adjustment) {
        g_signal_connect_object(
            self->adjustment, "value-changed", G_CALLBACK(adjustment_value_changed_cb), self,
            G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->adjustment, "notify::page-size", G_CALLBACK(update_visible), self,
            G_CONNECT_SWAPPED
        );
    }
...
static void
start_reordering(BrkTabBox *self, TabInfo *info) {
...
static void
reorder_animation_value_cb(double value, TabInfo *dest_tab) {
...
animate_reordering(BrkTabBox *self, TabInfo *dest_tab) {
    ...
    g_signal_connect_swapped(
        self->reorder_animation, "done", G_CALLBACK(reorder_animation_done_cb), self
    );
...
static void
reorder_offset_animation_value_cb(double value, TabInfo *info) {
...
static void
animate_reorder_offset(BrkTabBox *self, TabInfo *info, double offset) {
    gboolean is_rtl = gtk_widget_get_direction(GTK_WIDGET(self)) == GTK_TEXT_DIR_RTL;
    BrkAnimationTarget *target;
    double start_offset;
    ...
    g_signal_connect_swapped(
        info->reorder_animation, "done", G_CALLBACK(reorder_offset_animation_done_cb), info
    );
...
static void
page_reordered_cb(BrkTabBox *self, BrkTabPage *page, int index) {
    GList *link;
    int original_index;
...
static gboolean
drag_autoscroll_cb(GtkWidget *widget, GdkFrameClock *frame_clock, BrkTabBox *self) {
    double value, page_size;
    ...
    x = CLAMP(x, autoscroll_area, self->allocated_width - tab_width - autoscroll_area);
...
end_autoscroll(BrkTabBox *self) {
    if (self->drag_autoscroll_cb_id) {
        gtk_widget_remove_tick_callback(GTK_WIDGET(self), self->drag_autoscroll_cb_id);
...
static void
reorder_begin_cb(BrkTabBox *self, double start_x, double start_y, GtkGesture *gesture) {
...
static gboolean
gtk_drag_check_threshold_double(
    ...
    return (ABS(current_x - start_x) > drag_threshold || ABS(current_y - start_y) > drag_threshold);
}

static gboolean
check_dnd_threshold(BrkTabBox *self, double x, double y) {
    int threshold;
    graphene_rect_t rect;

    g_object_get(
        gtk_widget_get_settings(GTK_WIDGET(self)), "gtk-dnd-drag-threshold", &threshold, NULL
    );

    threshold *= DND_THRESHOLD_MULTIPLIER;

    graphene_rect_init(&rect, 0, 0, self->allocated_width, gtk_widget_get_height(GTK_WIDGET(self)));
...
static void
begin_drag(BrkTabBox *self, GdkDevice *device);

static void
reorder_update_cb(BrkTabBox *self, double offset_x, double offset_y, GtkGesture *gesture) {
    ...
    if (self->pressed_tab && self->pressed_tab != self->reorder_placeholder &&
        self->pressed_tab->page && !is_touchscreen(gesture) &&
        brk_tab_view_get_n_pages(self->view) > 1 && check_dnd_threshold(self, x, y)) {
...
static void
reorder_end_cb(BrkTabBox *self, double offset_x, double offset_y, GtkGesture *gesture) {
...
static void
select_page(BrkTabBox *self, BrkTabPage *page) {
    if (!page) {
...
static gboolean
extra_drag_drop_cb(BrkTab *tab, GValue *value, GdkDragAction current_action, BrkTabBox *self) {
...
extra_drag_value_cb(BrkTab *tab, GValue *value, BrkTabBox *self) {
...
static void
measure_tab(
    BrkGizmo *widget, GtkOrientation orientation, int for_size, int *minimum, int *natural,
...
static void
allocate_tab(BrkGizmo *widget, int width, int height, int baseline) {
    ...
    gtk_widget_allocate(
        child, width + width_diff, height, baseline,
...
create_tab_info(BrkTabBox *self, BrkTabPage *page) {
    ...
    brk_tab_setup_extra_drop_target(
        info->tab, self->extra_drag_actions, self->extra_drag_types, self->extra_drag_n_types
    );
...
static void
page_attached_cb(BrkTabBox *self, BrkTabPage *page, int position) {
    BrkAnimationTarget *target;
    TabInfo *info;
    GList *l;
    ...
    info->notify_needs_attention_id = g_signal_connect_object(
        page, "notify::needs-attention", G_CALLBACK(update_visible), self, G_CONNECT_SWAPPED
    );
    ...
    g_signal_connect_swapped(
        info->appear_animation, "done", G_CALLBACK(open_animation_done_cb), info
    );
...
static void
page_detached_cb(BrkTabBox *self, BrkTabPage *page) {
    BrkAnimationTarget *target;
    TabInfo *info;
    GList *page_link;
    ...
    g_signal_connect_swapped(
        info->appear_animation, "done", G_CALLBACK(close_animation_done_cb), info
    );
...
G_DECLARE_FINAL_TYPE(
    BrkTabBoxRootContent, brk_tab_box_root_content, BRK, TAB_BOX_ROOT_CONTENT, GdkContentProvider
)
...
static void
brk_tab_box_root_content_write_mime_type_async(
    GdkContentProvider *provider, const char *mime_type, GOutputStream *stream, int io_priority,
...
static gboolean
brk_tab_box_root_content_write_mime_type_finish(
...
static int
calculate_placeholder_index(BrkTabBox *self, int x) {
...
static void
insert_animation_value_cb(double value, TabInfo *info) {
    BrkTabBox *self = info->box;
...
static void
insert_placeholder(BrkTabBox *self, BrkTabPage *page, int pos) {
    ...
    if (info) {
        ...
        if (self->adjustment) {
            ...
            if (self->allocated_width > page_size) {
                gtk_widget_measure(
                    info->container, GTK_ORIENTATION_HORIZONTAL, -1, NULL,
                    &self->placeholder_scroll_offset, NULL, NULL
                );
...
static void
replace_placeholder(BrkTabBox *self, BrkTabPage *page) {
    TabInfo *info = self->reorder_placeholder;
    ...
    g_signal_connect_swapped(
        info->appear_animation, "done", G_CALLBACK(replace_animation_done_cb), info
    );
...
remove_placeholder(BrkTabBox *self) {
    ...
    g_signal_connect_swapped(
        info->appear_animation, "done", G_CALLBACK(remove_animation_done_cb), info
    );
...
static void
do_drag_drop(BrkTabBox *self, BrkTabBox *source_tab_box) {
...
static gboolean
is_view_in_the_same_group(BrkTabBox *self, BrkTabView *other_view) {
    /* TODO when we have groups, this should do the actual check */
    return TRUE;
}

static void
drag_end(BrkTabBox *self, GdkDrag *drag, gboolean success) {
    g_signal_handlers_disconnect_by_data(drag, self);

    gdk_drag_drop_done(drag, success);

    if (!success) {
        brk_tab_view_attach_page(self->view, self->detached_page, self->detached_index);
...
static void
tab_drop_performed_cb(BrkTabBox *self, GdkDrag *drag) {
...
static void
tab_drag_cancel_cb(BrkTabBox *self, GdkDragCancelReason reason, GdkDrag *drag) {
...
static void
create_drag_icon(BrkTabBox *self, GdkDrag *drag) {
    DragIcon *icon;
    BrkAnimationTarget *target;
    ...
    gtk_drag_icon_set_child(GTK_DRAG_ICON(gtk_drag_icon_get_for_drag(drag)), GTK_WIDGET(icon->tab));
...
resize_drag_icon(BrkTabBox *self, int width) {
    ...
    brk_timed_animation_set_value_from(BRK_TIMED_ANIMATION(icon->resize_animation), icon->width);
    brk_timed_animation_set_value_to(BRK_TIMED_ANIMATION(icon->resize_animation), width);
...
begin_drag(BrkTabBox *self, GdkDevice *device) {
    ...
    drag =
        gdk_drag_begin(surface, device, content, GDK_ACTION_MOVE, self->reorder_x, self->reorder_y);

    g_object_set_data(G_OBJECT(drag), "brk-tab-bar-drag-origin", self);

    g_signal_connect_swapped(drag, "drop-performed", G_CALLBACK(tab_drop_performed_cb), self);
    g_signal_connect_swapped(drag, "dnd-finished", G_CALLBACK(tab_dnd_finished_cb), self);
    g_signal_connect_swapped(drag, "cancel", G_CALLBACK(tab_drag_cancel_cb), self);
    ...
    gtk_widget_measure(
        detached_tab, GTK_ORIENTATION_HORIZONTAL, -1, NULL, &self->placeholder_scroll_offset, NULL,
        NULL
    );
    self->placeholder_scroll_offset /= 2;
...
static GdkDragAction
tab_drag_enter_motion_cb(BrkTabBox *self, double x, double y, GtkDropTarget *target) {
    ...
    if (!self->reorder_placeholder || !self->reorder_placeholder->page) {
        BrkTabPage *page = source_tab_box->detached_page;
        double center =
            x - source_tab_box->drag_icon->hotspot_x + source_tab_box->drag_icon->width / 2;
...
static gboolean
tab_drag_drop_cb(BrkTabBox *self, const GValue *value, double x, double y, GtkDropTarget *target) {
    BrkTabBox *source_tab_box;
...
static gboolean
view_drag_drop_cb(BrkTabBox *self, const GValue *value, double x, double y, GtkDropTarget *target) {
    BrkTabBox *source_tab_box;
...
static void
drag_leave_cb(BrkTabBox *self, GtkDropControllerMotion *controller) {
...
static void
drag_enter_motion_cb(BrkTabBox *self, double x, double y, GtkDropControllerMotion *controller) {
    TabInfo *info;
    GdkDrop *drop = gtk_drop_controller_motion_get_drop(controller);
    GdkDrag *drag = gdk_drop_get_drag(drop);
    BrkTabBox *source;
...
static void
do_popup(BrkTabBox *self, TabInfo *info, double x, double y) {
...
static void
long_pressed_cb(BrkTabBox *self, double x, double y, GtkGesture *gesture) {
    TabInfo *info;
...
static void
handle_click(BrkTabBox *self, TabInfo *info, GtkGesture *gesture) {
...
static void
pressed_cb(BrkTabBox *self, int n_press, double x, double y, GtkGesture *gesture) {
    TabInfo *info;
    GdkEvent *event;
...
static void
released_cb(BrkTabBox *self, int n_press, double x, double y, GtkGesture *gesture) {
    TabInfo *info;
    guint button;
...
static void
measure_tab_box(
    BrkTabBox *self, GtkOrientation orientation, int *minimum, int *natural, gboolean animated
) {
    int min, nat;
    ...
    if (orientation == GTK_ORIENTATION_HORIZONTAL) {
        ...
        for (l = self->tabs; l; l = l->next) {
            ...
            gtk_widget_measure(
                info->container, orientation, -1, &child_min, &child_nat, NULL, NULL
            );
        ...
        gtk_widget_measure(
            self->needs_attention_left, orientation, -1, &child_min, NULL, NULL, NULL
        );

        min = MAX(min, child_min);

        gtk_widget_measure(
            self->needs_attention_right, orientation, -1, &child_min, NULL, NULL, NULL
        );
...
static void
brk_tab_box_size_allocate(GtkWidget *widget, int width, int height, int baseline) {
    BrkTabBox *self = BRK_TAB_BOX(widget);
    gboolean is_rtl;
    GList *l;
    ...
    brk_tab_box_measure(
        widget, GTK_ORIENTATION_HORIZONTAL, -1, &self->allocated_width, NULL, NULL, NULL
    );
    ...
    gtk_adjustment_configure(
        self->adjustment, value, 0, self->allocated_width, width * 0.1, width * 0.9, width
    );
    ...
    gtk_widget_measure(
        self->needs_attention_left, GTK_ORIENTATION_HORIZONTAL, -1, &indicator_size, NULL, NULL,
        NULL
    );
    gtk_widget_allocate(self->needs_attention_left, indicator_size, height, baseline, NULL);

    gtk_widget_measure(
        self->needs_attention_right, GTK_ORIENTATION_HORIZONTAL, -1, &indicator_size, NULL, NULL,
        NULL
    );
...
static void
snapshot_tabs(BrkTabBox *self, GtkSnapshot *snapshot) {
...
static void
brk_tab_box_snapshot(GtkWidget *widget, GtkSnapshot *snapshot) {
    BrkTabBox *self = BRK_TAB_BOX(widget);
    double value = gtk_adjustment_get_value(self->adjustment);
    double page_size = gtk_adjustment_get_page_size(self->adjustment);
    double upper = gtk_adjustment_get_upper(self->adjustment);
    ...
    if (fadeLeft || fadeRight) {
        ...
        if (fadeLeft) {
            ...
            gtk_snapshot_append_linear_gradient(
                snapshot, &GRAPHENE_RECT_INIT(0, 0, FADE_OFFSET + FADE_WIDTH, height),
        ...
        if (fadeRight) {
            ...
            gtk_snapshot_append_linear_gradient(
                snapshot,
                &GRAPHENE_RECT_INIT(
                    width - FADE_OFFSET - FADE_WIDTH, 0, FADE_OFFSET + FADE_WIDTH, height
                ),
...
static void
brk_tab_box_direction_changed(GtkWidget *widget, GtkTextDirection previous_direction) {
    BrkTabBox *self = BRK_TAB_BOX(widget);
...
static void
brk_tab_box_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
    BrkTabBox *self = BRK_TAB_BOX(object);
...
static void
brk_tab_box_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    BrkTabBox *self = BRK_TAB_BOX(object);
...
brk_tab_box_class_init(BrkTabBoxClass *klass) {
    ...
    props[PROP_TAB_BAR] = g_param_spec_object(
        "tab-bar", NULL, NULL, BRK_TYPE_TAB_BAR,
        G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS
    );

    props[PROP_VIEW] = g_param_spec_object(
        "view", NULL, NULL, BRK_TYPE_TAB_VIEW,
        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY
    );

    props[PROP_RESIZE_FROZEN] = g_param_spec_boolean(
        "resize-frozen", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS
    );
    ...
    signals[SIGNAL_STOP_KINETIC_SCROLLING] = g_signal_new(
        "stop-kinetic-scrolling", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
        brk_marshal_VOID__VOID, G_TYPE_NONE, 0
    );
    g_signal_set_va_marshaller(
        signals[SIGNAL_STOP_KINETIC_SCROLLING], G_TYPE_FROM_CLASS(klass), brk_marshal_VOID__VOIDv
    );

    signals[SIGNAL_EXTRA_DRAG_DROP] = g_signal_new(
        "extra-drag-drop", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
        g_signal_accumulator_first_wins, NULL, NULL, G_TYPE_BOOLEAN, 3, BRK_TYPE_TAB_PAGE,
        G_TYPE_VALUE, GDK_TYPE_DRAG_ACTION
    );

    signals[SIGNAL_EXTRA_DRAG_VALUE] = g_signal_new(
        "extra-drag-value", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
        g_signal_accumulator_first_wins, NULL, NULL, GDK_TYPE_DRAG_ACTION, 2, BRK_TYPE_TAB_PAGE,
        G_TYPE_VALUE
    );
    ...
    gtk_widget_class_add_binding_action(
        widget_class, GDK_KEY_F10, GDK_SHIFT_MASK, "menu.popup", NULL
    );
...
brk_tab_box_init(BrkTabBox *self) {
    ...
    g_signal_connect_swapped(
        self->resize_animation, "done", G_CALLBACK(resize_animation_done_cb), self
    );
   ...
   */
    ...
    g_signal_connect_swapped(
        self->scroll_animation, "done", G_CALLBACK(scroll_animation_done_cb), self
    );

    self->needs_attention_left = gtk_revealer_new();
    gtk_revealer_set_transition_type(
        GTK_REVEALER(self->needs_attention_left), GTK_REVEALER_TRANSITION_TYPE_CROSSFADE
    );
    ...
    gtk_revealer_set_transition_type(
        GTK_REVEALER(self->needs_attention_right), GTK_REVEALER_TRANSITION_TYPE_CROSSFADE
    );
...
void
brk_tab_box_set_view(BrkTabBox *self, BrkTabView *view) {
    ...
    if (self->view) {
        ...
        g_signal_connect_object(
            self->view, "page-detached", G_CALLBACK(page_detached_cb), self, G_CONNECT_SWAPPED
        );
        g_signal_connect_object(
            self->view, "page-reordered", G_CALLBACK(page_reordered_cb), self, G_CONNECT_SWAPPED
        );

        g_signal_connect_object(
            self->view, "notify::n-pages", G_CALLBACK(update_single_tab_style), self,
            G_CONNECT_SWAPPED
        );

        update_single_tab_style(self);

        self->view_drop_target =
            GTK_EVENT_CONTROLLER(gtk_drop_target_new(BRK_TYPE_TAB_PAGE, GDK_ACTION_MOVE));

        g_signal_connect_object(
            self->view_drop_target, "drop", G_CALLBACK(view_drag_drop_cb), self, G_CONNECT_SWAPPED
        );
...
void
brk_tab_box_attach_page(BrkTabBox *self, BrkTabPage *page, int position) {
    g_return_if_fail(BRK_IS_TAB_BOX(self));
...
void
brk_tab_box_detach_page(BrkTabBox *self, BrkTabPage *page) {
    g_return_if_fail(BRK_IS_TAB_BOX(self));
    g_return_if_fail(BRK_IS_TAB_PAGE(page));
...
void
brk_tab_box_select_page(BrkTabBox *self, BrkTabPage *page) {
    g_return_if_fail(BRK_IS_TAB_BOX(self));
...
gboolean
brk_tab_box_is_page_focused(BrkTabBox *self, BrkTabPage *page) {
    TabInfo *info;
...
void
brk_tab_box_setup_extra_drop_target(
    BrkTabBox *self, GdkDragAction actions, GType *types, gsize n_types
) {
    GList *l;
...
void
brk_tab_box_set_expand_tabs(BrkTabBox *self, gboolean expand_tabs) {
    g_return_if_fail(BRK_IS_TAB_BOX(self));
...
void
brk_tab_box_set_inverted(BrkTabBox *self, gboolean inverted) {
    GList *l;
...
void
brk_tab_box_set_extra_drag_preload(BrkTabBox *self, gboolean preload) {
    GList *l;
