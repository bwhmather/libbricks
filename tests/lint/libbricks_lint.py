...
def enumerate_object_paths():
    paths = [object_path for object_path in BUILD_ROOT.glob("src/**/*.o")]
    yield from sorted(paths)
...
def is_libbricks_source_path(path, /):
    assert path.is_absolute()
...
def is_libbricks_header_path(path, /):
    assert path.is_absolute()
...
def is_libbricks_object_path(path, /):
    assert path.is_absolute()
...
def resolve_include_path(include_path, /, *, source_path=None):
    if source_path is None:
        source_path = SOURCE_ROOT / "bricks.c"
...
def derive_include_from_path(include_path, /):
    ...
    for candidate_dir in include_dirs(SOURCE_ROOT / "bricks.c"):
